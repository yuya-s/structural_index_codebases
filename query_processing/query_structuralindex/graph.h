...
inline bool cmpedgedst(const Edge &p1, const Edge &p2){
	if(p1.dst < p2.dst)return true;
	else if(p1.dst==p2.dst&&p1.src < p2.src)return true;
    else if(p1.dst==p2.dst&&p1.src==p2.src&&p1.label < p2.label)return true;
	return false;
}
